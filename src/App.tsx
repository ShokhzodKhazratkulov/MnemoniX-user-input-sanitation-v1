...
export default function App() {
  ...
  const handleSearch = async (e?: React.FormEvent, word?: string) => {
    ...
    try {
      // 2. Native Language Detection & Translation
      // We use Gemini to translate or validate if it's a valid English word/phrase
      const translatedWord = await gemini.translateToEnglish(query);
      
      if (translatedWord.toUpperCase() === 'REJECTED') {
        setError(t.errorInvalidInput);
        setState(AppState.ERROR);
        return;
      }

      const englishWord = translatedWord.toLowerCase().trim();
      ...
      // Check cache with the English word
      const cacheKey = `${englishWord}-${contentLanguage}`;
      ...
      if (!existingMnemonic) {
        // 4. If not found in DB immediately, double check spelling (for English edge cases)
        ...
        if (!existingMnemonic) {
          // 5. Generate new mnemonic for the English word
