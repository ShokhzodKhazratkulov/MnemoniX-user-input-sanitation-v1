...
3. Return ONLY a valid JSON object.`
   ...
   */
  async translateToEnglish(input: string): Promise<string> {
    return this.withRetry(async () => {
      const ai = this.getAI();
      const response = await ai.models.generateContent({
        model: "gemini-3-flash-preview",
        contents: `Translate the following input to a single English word or a short, meaningful English phrase (like "business equity").
        
        Input: "${input}"
        
        Rules:
        1. If the input is already in English, return it as is.
        2. If the input is in Uzbek, Russian, Kazakh, Kyrgyz, Tajik, or Turkmen, translate it to the most accurate English equivalent.
        3. If the input is a full sentence, a long paragraph, or nonsensical gibberish, return the word "REJECTED".
        4. Return ONLY the translated English word/phrase or "REJECTED".
        5. Do not include punctuation, explanations, or quotes.`,
        config: {
          thinkingConfig: { thinkingLevel: ThinkingLevel.LOW }
        }
      });

      const result = response.text?.trim() || "REJECTED";
      return result;
    });
  }
}
